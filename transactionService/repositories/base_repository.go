...
type FindAllWhereOptions struct {
	Limit     *int
	Skip      *int
	Where     interface{}
	Relations []string
...
type Repository[T any] interface {
	...
	FindAllWhere(options FindAllWhereOptions) *[]T
...
func (r repository[T]) FindAllWhere(options FindAllWhereOptions) *[]T {
	var entities []T
	tx := r.db.Where(options.Where)
	if options.Limit != nil {
		tx = tx.Limit(*options.Limit).Offset(*options.Skip)
	}
	tx = preloadColumns(tx, &options.Relations)
	err := tx.Find(&entities).Error
	if err != nil {
...
func (r repository[T]) FindAllWhereOr(query *[]T, limit int, skip int, preload *[]string) *[]T {
	var entities []T
	tx := r.db.Where((*query)[0])
	if len(*query) > 1 {
		for i := 1; i < len(*query); i++ {
			tx = tx.Or((*query)[i])
		}
	}
	tx = tx.Limit(limit).Offset(skip)
	tx = preloadColumns(tx, preload)
	err := tx.Find(&entities).Error
	if err != nil {
		return &[]T{}
